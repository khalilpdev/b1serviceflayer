  ...
  group('Low Level SL Tests', () {
    ...
    test('ErrorHandlingForInvalidPropertyName', () async {
      try {
        ...
        if (e is B1Error) {
          print(
              'Exception is B1Error (${e.statusCode}) ${e.error.message.value} (${e.error.code}) for Query ${e.queryUrl} Payload ${e.postBody}');
        }
    ...
    test('ErrorHandlingForInvalidDocStatusCode', () async {
      try {
        await b1s.queryAsync(
            "PurchaseOrders?\$filter=DocumentStatus eq 'bost_OpenX'");
      } catch (e) {
        if (e is B1Error) {
          print(
              'Exception is B1Error (${e.statusCode}) ${e.error.message.value} (${e.error.code}) for Query ${e.queryUrl} Payload ${e.postBody}');
        ...
        const expectedErrorCode =
            -1013; // Invalid item name bost_OpenX in Enum BoStatus The valid names are: bost_Open-O, bost_Close-C, bost_Paid-P, bost_Delivered-D
        expect(b1s.b1Error.error.code, equals(expectedErrorCode));
      }
    });
  });
